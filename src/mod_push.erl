...
-type notify_failure_callback() :: fun((error | timeout, term()) -> any()).
...
disable(#jid{luser = LUser, lserver = LServer, lresource = LResource} = JID,
	...
	PID when is_pid(PID) ->
		?INFO_MSG("Disabling push notifications Server: ~ts JID: ~ts Token: ~ts",
			      [jid:encode(PushJID),jid:encode(JID),Node]),
...
notify(LServer, PushLJID, Node, XData, Pkt0, Dir, FailureCallback) ->
    ...
    case {make_summary(LServer, Pkt, Dir), mod_push_opt:notify_on(LServer)} of
        {undefined, messages} ->
            ...
            ok;
        {Summary, _NotifyOn} ->
	    {NotificationType, ProposeID, ProposeMedia} =              
                case extract_jingle_info(Pkt) of
                    {ok, ID, Media} ->
                        {<<"call">>, ID, Media};
                    error ->
                        {<<"message">>, <<"">>, <<"">>}
	    	...
	    	?DEBUG("NotificationType", [NotificationType]),            

            % Extraer todos los fields de XData
            XData_Fields = XData#xdata.fields,
             % Extract the fields from the Summary xdata record
			SummaryFields = case Summary of
            ...
            % Combinar los campos existentes con los nuevos
  		    CombinedFields = XData_Fields ++ NewFields ++ SummaryFields,
            ?DEBUG("mod_push CombinedFields", [CombinedFields]),
                                
			PushServerBin = case PushLJID of
				S when is_binary(S) -> S;
				{_, S, _} when is_binary(S) -> S;
				_ -> <<>>
			end,
			
			process_push_notification(PushServerBin,Node,CombinedFields,FailureCallback)
    end.

-spec process_push_notification(binary(), binary(), [#xdata_field{}],
...
process_push_notification(PushServer,Node, Fields,FailureCallback) ->
	...
	Title = find_field_value(<<"last-message-sender">>, Fields),
	Body = find_field_value(<<"last-message-body">>, Fields),
...
%% Para asegurar valores binarios
-spec ensure_binary(binary() | string() | atom() | undefined) -> binary().
...
-spec extract_jingle_info(xmpp_element()) -> {ok, binary(), binary()} | error.
extract_jingle_info(#message{sub_els = SubEls}) ->
    case lists:keyfind(<<"propose">>, #xmlel.name, SubEls) of
        #xmlel{name = <<"propose">>, attrs = ProposeAttrs, children = ProposeChildren} ->
            case {proplists:get_value(<<"xmlns">>, ProposeAttrs),
                  proplists:get_value(<<"id">>, ProposeAttrs)} of
                {<<"urn:xmpp:jingle-message:0">>, ProposeID} when is_binary(ProposeID) ->
                    case lists:keyfind(<<"description">>, #xmlel.name, ProposeChildren) of
                        #xmlel{attrs = DescAttrs} ->
                            case proplists:get_value(<<"media">>, DescAttrs) of
